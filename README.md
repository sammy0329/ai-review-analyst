...
| **시맨틱 캐싱**      | 유사 질문 캐싱으로 API 비용 절감, 응답 2.8초→0.4초 (85% 단축)    | `Vector Similarity`, `Caching`       |
| **사용자 피드백**    | Q&A 답변에 👍/👎 버튼, 피드백 통계 수집                          | `Feedback Loop`, `Analytics`         |
...
프로덕션 환경을 고려한 **비용 최적화**와 **성능 개선** 전략을 구현했습니다.

### 1. 시맨틱 캐싱 (Semantic Caching)

LLM API 호출 비용을 절감하고 응답 속도를 획기적으로 향상시키기 위해 **유사 질문 캐싱 시스템**을 구현했습니다.

```
[캐시 동작 흐름]
질문 입력 → 임베딩 변환 → 유사도 검색 (ChromaDB)
    ↓
유사도 ≥ 70% → 캐시 히트 → 캐시 반환 (0.4초)
유사도 < 70% → 캐시 미스 → LLM 호출 → 캐시 저장 → 반환 (2.8초)
```
...
**구현 특징:**
- **제품별 캐시 분리:** 각 제품의 Q&A 컨텍스트가 독립적으로 관리
- **TTL 7일:** 리뷰 데이터 변경 시 자동 갱신
- **70% 유사도 임계값:** 동일 의도의 질문을 효과적으로 캐싱

**유사도 테스트 예시** (기준 질문: "배송 빠른가요?"):
| 질문 | 유사도 | 캐시 히트 |
|------|--------|-----------|
| "배송 빠른가요?" | 100% | ✅ |
| "배송이 빠른지 알려주세요" | 81.2% | ✅ |
...
→ 70% 임계값으로 동일 의도 질문은 캐시 히트, 다른 주제는 캐시 미스

### 2. 검색 품질 개선 (Query Expansion)

사용자의 형식적인 질문을 자연어 리뷰와 매칭하기 위한 **쿼리 확장** 기법을 적용했습니다.

| 원본 쿼리 | 확장된 쿼리 | 유사도 개선 |
|-----------|-------------|-------------|
| "장점은 무엇인가요?" | "좋은 점이 뭐예요?" | 0.179 → **0.246** |
| "단점은 무엇인가요?" | "아쉬운 점이 뭐예요?" | 부정 리뷰 정확도 향상 |

### 3. 사용자 피드백 루프 (Feedback Collection)

응답 품질 검증과 지속적 개선을 위한 **피드백 수집 시스템**을 구축했습니다.
...
**수집 데이터:**
- 제품명, 질문, 답변, 피드백(helpful/not helpful), 타임스탬프
- 제품별/전체 피드백 통계 조회 API

### 4. 사용 메트릭 대시보드 (Usage Analytics)

데이터 기반 의사결정을 위한 **실시간 사용 통계**를 수집합니다.
...
### Phase 8: Optimization & Validation (100% 완료)

- [x] **사용자 피드백 수집**: Q&A 답변에 👍/👎 버튼, 피드백 통계 수집
- [x] **사용 메트릭 대시보드**: 질문 로그, 응답 시간 측정, 인기 키워드 분석
- [x] **시맨틱 캐싱**: 유사 질문 캐싱 (70% 유사도), API 비용 절감, 응답 2.8초→0.4초 (85% 단축)
- [x] **검색 품질 개선**: 쿼리 확장 (장점→좋은 점), threshold 최적화 (0.1)
