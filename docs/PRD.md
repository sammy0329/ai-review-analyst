...
| **문서 버전**   | v1.3                  | **작성자**      | 안성재                                                       |
| --------------- | --------------------- | --------------- | ------------------------------------------------------------ |
| **상태**        | 개발 완료 (Phase 1-8) | **타겟 플랫폼** | Web (MVP), App Integration                                   |
| **최종 수정일** | 2026-01-16            | **GitHub**      | [Repository](https://github.com/sammy0329/ai-review-analyst) |
...
### F6. 시맨틱 캐싱 (Semantic Caching)

**설명:** 유사한 질문에 대해 캐시된 답변을 반환하여 API 비용을 절감하고 응답 속도를 획기적으로 향상시킵니다.

**핵심 기능:**

- **유사도 기반 캐시 조회:** 질문 임베딩의 코사인 유사도 70% 이상이면 캐시 히트
- **제품별 캐시 분리:** 각 제품의 Q&A 캐시가 독립적으로 관리
- **TTL 기반 만료:** 7일 경과 시 자동 만료, 최신 리뷰 반영
- **캐시 통계 표시:** 히트율, 절감 비용 실시간 표시
...
**관련 기술:** `ChromaDB`, `Cosine Similarity`, `TTL Cache`

---

### F7. 사용자 피드백 수집 (Feedback Collection)

**설명:** Q&A 답변에 대한 사용자 피드백(👍/👎)을 수집하여 응답 품질을 검증하고 개선합니다.

**핵심 기능:**

- **피드백 버튼 UI:** AI 답변 후 👍(도움됨) / 👎(도움안됨) 버튼 표시
- **중복 방지:** 동일 답변에 중복 피드백 불가
- **피드백 통계:** 제품별/전체 피드백 통계 조회
...
**관련 기술:** `SQLite`, `Session State`, `Analytics`

---

### F8. 사용 메트릭 대시보드 (Usage Metrics)

**설명:** 데이터 기반 의사결정을 위한 사용 통계를 수집하고 표시합니다.

**핵심 기능:**

- **질문 로그 저장:** 제품별 질문 내용, 응답 시간 기록
- **응답 시간 측정:** 각 Q&A 호출의 소요 시간 측정
- **인기 키워드 분석:** 자주 질문되는 키워드 Top 3 표시

**표시 예시:**
```
📊 이 제품 4개 질문 · 평균 응답 9.5초 · 인기: 배송(3), 가성비(1)
```

**관련 기술:** `SQLite`, `Text Analytics`, `Real-time Stats`
