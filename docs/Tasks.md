...
| Task ID | Task | 상태 | 설명 |
|---------|------|------|------|
| 1.4.1 | [ ] 리뷰 텍스트 정제 함수 | 대기 | 특수문자, 이모지 처리 |
| 1.4.2 | [ ] 텍스트 청킹(Chunking) 로직 | 대기 | 의미 단위 분할 |
| 1.4.3 | [ ] 메타데이터 추출 | 대기 | 별점, 날짜, 구매옵션 |
| 1.4.4 | [ ] 중복 리뷰 필터링 | 대기 | 해시 기반 중복 제거 |
| 1.4.5 | [ ] 전처리 파이프라인 클래스 | 대기 | `src/pipeline/preprocessor.py` |
| 1.4.6 | [ ] 전처리 단위 테스트 | 대기 | `tests/test_preprocessor.py` |
