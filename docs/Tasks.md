...
| Task ID | Task | 상태 | 설명 |
|---------|------|------|------|
| 7.3.1 | [x] 임의 날짜 생성 | 완료 | 작년~현재 사이 임의 날짜 생성 |
| 7.3.2 | [x] 대표 리뷰 선정 로직 | 완료 | 신뢰도 높은 리뷰 우선 선정 |

**완료 기준:** 리뷰 데이터가 실제 서비스처럼 표시

**구현 내용:**
- `randomize_review_dates()`: 모든 리뷰에 임의 날짜 설정 (365일 전 ~ 현재)
- 대표 리뷰: `is_suspicious=False`인 리뷰만 선정 (Epic 7.2에서 구현)
...
## Phase 8: Optimization & Validation (최적화 및 검증)
...
### Epic 8.2: 사용 메트릭 대시보드 ✅

> **목표:** 데이터 기반 의사결정을 위한 사용 통계 수집

| Task ID | Task | 상태 | 설명 |
|---------|------|------|------|
| 8.2.1 | [x] 질문 로그 테이블 생성 | 완료 | qa_logs 테이블 (product, question, response_time_ms, timestamp) |
| 8.2.2 | [x] 질문 로그 저장 로직 | 완료 | Q&A 호출 시 자동 저장 + 응답 시간 측정 |
| 8.2.3 | [x] 메트릭 집계 함수 | 완료 | get_qa_metrics() - 제품별 질문 수, 평균 응답 시간, 인기 키워드 Top 3 |
| 8.2.4 | [x] Q&A 섹션 메트릭 표시 | 완료 | Q&A 헤더에 제품별 통계 표시 |

**완료 기준:** 사용 통계를 실시간으로 확인 가능 ✅

**구현 내용:**
- `src/database.py`: qa_logs 테이블, save_qa_log(), get_qa_metrics(product_name)
- `src/ui/app.py`: Q&A 호출 시 응답 시간 측정 및 로그 저장
- Q&A 헤더에 **제품별** 통계 표시 (전역 아님)
- 인기 질문 키워드 Top 3 분석 (배송, 품질, 가성비, 사이즈, 기능)
...
> **목표:** 유사 질문 캐싱으로 API 비용 절감 및 응답 속도 향상

| Task ID | Task | 상태 | 설명 |
|---------|------|------|------|
| 8.3.1 | [x] 캐시 컬렉션 생성 | 완료 | ChromaDB `qa_cache` 컬렉션 |
| 8.3.2 | [x] 유사 질문 검색 로직 | 완료 | cosine similarity > 0.95 |
| 8.3.3 | [x] 캐시 히트/미스 처리 | 완료 | 히트 시 캐시 반환, 미스 시 생성 후 저장 |
| 8.3.4 | [x] 캐시 통계 표시 | 완료 | 히트율, 절감 비용 표시 |
| 8.3.5 | [x] 캐시 만료/무효화 | 완료 | TTL 7일, 제품별/전체 무효화 |

**완료 기준:** 유사 질문에 대해 캐시된 답변 반환 ✅

**구현 내용:**
- `src/pipeline/semantic_cache.py`: SemanticCache 클래스
  - ChromaDB `qa_cache` 컬렉션 (cosine similarity)
  - `lookup()`: 유사 질문 검색 (similarity > 0.95)
  - `store()`: 질문-답변 캐싱 (제품별 분리)
  - `invalidate()`, `invalidate_product()`: 캐시 무효화
  - `clear_expired()`: TTL 7일 기준 만료 캐시 정리
  - `get_stats()`: 히트율, 절감 비용 통계
- `src/ui/app.py`: Q&A에 캐시 통합
  - 캐시 히트 시 즉시 응답 (스트리밍 없이)
  - "⚡ 캐시에서 응답 (유사도 95%)" 표시
  - Q&A 헤더에 캐시 히트율/절감 비용 표시

**기대 효과:**
| 항목 | Before | After |
|------|--------|-------|
| API 비용 | 매 질문 $0.002 | 캐시 히트 시 $0 |
| 응답 속도 | 2~3초 | 캐시 히트 시 **0.1초** |

---

### Epic 8.4: 응답 품질 개선 ✅

> **목표:** 검색 품질 향상 및 답변 신뢰도 개선

| Task ID | Task | 상태 | 설명 |
|---------|------|------|------|
| 8.4.1 | [x] 검색 품질 개선 | 완료 | top_k 5→10, 유사도 threshold 0.3 적용 |
| 8.4.2 | ~~응답에 비용 표시~~ | 스킵 | 고객 화면에 부적합하여 제외 |
| 8.4.3 | [x] 출처 표시 개선 | 완료 | "📚 N개 리뷰 참고", 관련 리뷰 없음 처리 |

**구현 내용:**
- `RAGConfig.top_k` 5 → 10으로 증가
- `RAGConfig.min_score` 0.3 추가 (유사도 임계값)
- `ReviewRAGChain._retrieve_filtered()` 메서드로 score 기반 필터링
- `render_qa_sources()`: "📚 N개 리뷰 참고" 표시
- 관련 리뷰 0개인 경우 "관련 리뷰를 찾지 못했습니다" 안내

**완료 기준:** 관련성 높은 리뷰만 검색되고, 출처 정보가 명확히 표시됨 ✅
...
| Phase 8: Optimization & Validation | 4 | 4 | 100% |
| **Total** | **30** | **28** | **93%** |
...
*최종 업데이트: 2026-01-15 (Epic 8.4: 응답 품질 개선 완료)*
