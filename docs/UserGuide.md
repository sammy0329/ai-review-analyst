...
### 주요 기능
- **AI 추천 판단**: 속성별 강점/약점 분석 기반 추천 여부 제시
- **Q&A**: 카카오톡 스타일 채팅으로 자연어 질문
- **가짜 리뷰 필터링**: 의심 리뷰 [의심] 라벨 표시, 신뢰할 수 있는 리뷰만 분석
- **근거 리뷰 표시**: AI 답변 시 참고한 리뷰 원문 + 핵심 문구 하이라이트
...
Q&A 탭에서 자연어로 질문할 수 있습니다. 카카오톡 스타일의 채팅 인터페이스로 대화합니다.
...
**답변 특징:**
- 질문 전송 시 즉시 표시되고, AI 응답 대기 중 로딩 스피너 표시
- 답변과 함께 **근거 리뷰** 확인 가능 (팝업 형식)
- 근거 리뷰에서 **핵심 문구 하이라이트** (노란색 배경)
- 의심 리뷰는 **[의심]** 라벨로 표시 (주황색)

### 4.4 근거 리뷰 확인

AI 답변 아래 "📚 근거 리뷰 (N개)" 버튼을 클릭하면:
- 답변 생성에 참고한 리뷰 원문 확인
- 각 리뷰의 별점, 감정(긍정/중립/부정), 속성 태그 표시
- AI가 인용한 문구가 노란색으로 하이라이트됨
- 가짜 리뷰 의심 시 [의심] 라벨과 사유 표시
...
- **GitHub Issues**: [https://github.com/sammy0329/ai-review-analyst/issues](https://github.com/sammy0329/ai-review-analyst/issues)
